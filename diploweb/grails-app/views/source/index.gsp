		...
		<div id="list-source" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="newPostsFinderModule" title="${message(code: 'source.newPostsFinderModule.label', default: 'New Posts Finder Module')}" />
					
						<g:sortableColumn property="rssUrl" title="${message(code: 'source.rssUrl.label', default: 'Rss Url')}" />
				...
				<g:each in="${sourceInstanceList}" status="i" var="sourceInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td>${fieldValue(bean: sourceInstance, field: "newPostsFinderModule")}</td>
					
						<td>${fieldValue(bean: sourceInstance, field: "rssUrl")}</td>
